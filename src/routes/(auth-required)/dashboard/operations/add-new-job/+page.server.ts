...
export const actions = {
	...
	/**
	 * Server-side action to update existing flight estimates
	 */
	updateFlightEstimates: async ({ request }) => {
		try {
			const formData = await request.formData();
			const jobno = formData.get('jobno') as string;
			...
			if ((!jobno && !jobnumber) || !flightRecommendationData || !originAirportCode || !destinationAirportCode) {
				return fail(400, { 
					error: 'Missing required fields: (jobno or jobnumber), flightRecommendation, originAirportCode, destinationAirportCode' 
				});
			}
			...
			// Update flight timing estimates in timetable (use jobno for timetable)
			const timetableResult = await saveFlightEstimatesToTimetable(
				jobno,
				flightRecommendation
			);
			...
			if (!timetableResult.success) {
				return fail(500, { error: timetableResult.error || 'Failed to update flight timing estimates' });
			}
			...
			console.error('Error in updateFlightEstimates action:', err);
			return fail(500, { error: 'Internal server error' });
		}
	}
} satisfies Actions;
