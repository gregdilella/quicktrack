...
<script lang="ts">
    ...
    let form = { name: '', type: '', temperature: '' }
    ...
    async function handleSubmit() {
        if (!validate()) { message = 'Please fix the validation errors below'; return }
        try {
            saving = true
            message = ''
            ...
            if (error) { console.error('Error creating packaging:', error); message = `Error creating packaging: ${error.message}`; return }

            message = 'Packaging created successfully!'
            setTimeout(() => goto('/dashboard/management/packaging-search'), 1200)
        } catch (err) {
...
CCCCCC EEEEEEE RR   RR   TT     UUUUU   SSSSS</pre>
        ...
        <div class="system-title"><span class="red-text">MANAGEMENT PORTAL - ADD NEW PACKAGING</span></div>

        <div class="nav-section"><a href="/dashboard/management" class="nav-link">⬅ BACK TO MANAGEMENT MENU</a></div>

        {#if user && userProfile}
            <div class="user-info">
                <p class="blue-text">Status: <span class="purple-text">MANAGEMENT ACCESS</span></p>
                <p class="blue-text">Function: PACKAGING REGISTRATION</p>
            </div>
        {/if}

        <div class="form-section">
            <h3 class="blue-text">--- NEW PACKAGING TYPE ---</h3>
            <form on:submit|preventDefault={handleSubmit} class="customer-form">
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label class="blue-text">TYPE:</label>
                        <input type="text" bind:value={form.type} class="form-input" />
                    </div>
                </div>

                <div class="form-row">
                    <div class="form-group">
                        <label class="blue-text">TEMPERATURE:</label>
                        <input type="text" bind:value={form.temperature} class="form-input" />
                    </div>
                </div>

                <div class="form-actions">
                    <button type="submit" class="submit-button" disabled={saving}>{saving ? 'CREATING PACKAGING...' : 'CREATE PACKAGING'}</button>
        ...
        <div class="command-prompt"><span class="red-text">([MGMT] Packaging Registration Module - Ready for Input)</span></div>
