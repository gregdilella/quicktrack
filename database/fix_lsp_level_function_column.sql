-- =====================================================
-- FIX LSP_LEVEL FUNCTION COLUMN - DIRECT FIX
-- Address VARCHAR(1) constraint issue
-- =====================================================
...
-- STEP 2: Drop the check constraint that's blocking changes
ALTER TABLE lsp_level DROP CONSTRAINT IF EXISTS lsp_level_function_check;

-- STEP 3: Convert any existing single-character values to full words
...
-- STEP 4: Alter the column type to allow full words
...
-- STEP 6: Clean up blank LSPs
DELETE FROM lsps 
WHERE vendor_name IS NULL 
   OR TRIM(vendor_name) = '' 
   OR LENGTH(TRIM(vendor_name)) = 0;

-- STEP 7: Add sample LSPs if table is empty
DO $$
BEGIN
...
-- STEP 8: Ensure AWB table exists
...
-- STEP 9: Ensure lsp_level_id exists in lsp_costs
DO $$ 
BEGIN
    IF NOT EXISTS (
...
-- Create simplified policies for operations users
...
-- STEP 12: Test LSP insertion
BEGIN;
INSERT INTO lsp_level (jobnumber, lsp_id, vendorcode, function, status, assigned_date) 
VALUES (
    'TEST-LSP-123', 
...
SELECT 'LSP_LEVEL_INSERT_TEST_SUCCESS' as test_result;
ROLLBACK;

-- Final verification
SELECT 
    'FUNCTION_COLUMN_FIXED' as status,
